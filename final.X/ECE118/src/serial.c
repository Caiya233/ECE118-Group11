...
{
    uint8_t wasEmpty = (TransmitTail == TransmitHead);
 ...
 * @param c - char to be sent
 * @return c - the char that was sent
 * @brief  overwrites weakly defined extern to use circular buffer instead of Microchip 
 * functions
 * @author Gabriel Elkaim, 2026.03.28 */
int putchar(int c)
{
    if (c == '\n') {
        PutChar('\r');
...
/**
 * @Function write(int handle, const void *buffer, size_t len)
 * @param handle - unused variable but required to match standard library form
 ...
 * functions
 * @author Gabriel Elkaim, 2026.03.28 */
ssize_t write(int handle, const void *buffer, size_t len)
{
...
void __ISR(_UART1_VECTOR, IPL4SOFT) IntUart1Handler(void)
{
    ...
    if (IFS0bits.U1TXIF) {
        IFS0bits.U1TXIF = 0;
        if ((!AddingToTransmit)) {
            while ((TransmitTail != TransmitHead) && (!U1STAbits.UTXBF)) {
