...
char RC_Init(void)
{
    ...
    IPC4bits.T4IP = 4;
    IEC0bits.T4IE = 1; // start interrupts for timer
...
void __ISR(_TIMER_4_VECTOR, IPL4SOFT) Timer4IntHandler(void)
{
    ...
    case pulsing:
        ...
        RC_SetPin(indexPin);
        PR4=TMR4+(uSEC * RCupTime[indexPin]);
    ...
    case idling:
        ...
        if (uSecondsLeftToGo <= 0) {
            ...
            PR4=(currentTime + uSEC * uSecondsLeftToGo);
            RCstate = pulsing;
        } else { // need to cycle through
            uSecondsLeftToGo -= MAXPULSE;
            PR4=(currentTime + uSEC * MAXPULSE);
        }
